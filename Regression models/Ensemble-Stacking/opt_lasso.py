...
def lasso_parameters():
    params = {'final_estimator': [Lasso(fit_intercept=False, positive=True, alpha=1),
                                  Lasso(fit_intercept=False, positive=True, alpha=0.3),
                                  Lasso(fit_intercept=False, positive=True, alpha=0.1),
                                  Lasso(fit_intercept=False, positive=True, alpha=0.03),
                                  Lasso(fit_intercept=False, positive=True, alpha=0.01),
                                  Lasso(fit_intercept=False, positive=True, alpha=0.003),
                                  Lasso(fit_intercept=False, positive=True, alpha=0.001),
                                  Lasso(fit_intercept=False, positive=True, alpha=0.0003),
                                  Lasso(fit_intercept=False, positive=True, alpha=0.0001)]}
    fine_params = {}
...
def model_adjust_parameters(cv_params, other_params):
    ...
    parameters_score.to_excel('./opt_lasso.xlsx', index=False)

    print('Optimization finished')


if __name__ == '__main__':
    adj_params, fixed_params = lasso_parameters()
